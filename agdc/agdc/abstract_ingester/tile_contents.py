...
class TileContents(object):
    ...
    def has_data(self):
        if not os.path.exists(self._temp_tile_output_path):
            ...
            print 'No reprojected tile has been produced.'
            return False
        #else:
        #    return True
    ...
    def remove(self):
        ...
        if os.path.isfile(self._temp_tile_output_path):
            LOGGER.info('Temp dir: '+self._temp_tile_output_path)
            shutil.copy(self._temp_tile_output_path,'/tilestore/data2/')
...
CellSet = set()
# PART 1
# Add all the cells to a set
with open("/tilestore/world_cells.txt") as f:
    for line in f:
        (x,y) = line.split()
        CellSet.add((int(x),int(y)))
# PART 3
# Check if a given tile  exists in the set
def _is_valid(x,y):
    global CellSet
    tile = (x,y)
    if tile in CellSet:
...
def _create_reproject_command(band_stack, first_file_number, nodata_value, temp_tile_output_path, tile_footprint,
    ...
    #tile_extents=(100.0,-100.0,150.0,-150.0)
    print "LL: "+str(tile_extents[0])+", "+str(tile_extents[1])
    #Not efficient, but it works for now
    if not _is_valid(int(tile_extents[0]),int(tile_extents[1])):
        return []
...
def _has_data(tile_path, band_stack):
    ...
    for band_index in range(tile_dataset.RasterCount):
        ...
        tmp_ndv = band.GetNoDataValue()
        LOGGER.info("dataset ndv: "+str(nodata_val)+" band ndv: "+str(tmp_ndv))
        if nodata_val is not None and nodata_val !=tmp_ndv:
            LOGGER.info('No data value is not correct! Dataset set:'+str(nodata_val)+' and band: '+str(tmp_ndv))
        del tmp_ndv
        ...
        if nodata_val is None:
            LOGGER.info('Special case for PQA with no no-data value defined')
            if (band_stack.band_dict[file_number]['level_name'] == 'PQA'):
                ...
                LOGGER.info('nodata_value of None means all array data is valid')
